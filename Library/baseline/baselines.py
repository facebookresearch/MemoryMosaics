...
# The GPT classes are modified from https://github.com/karpathy/nanoGPT 
import math, os
import numpy as np
import logging
import torch
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        # #
        # droprate = 0.7
        # mask = (torch.rand(size = x.shape,device=x.device) >= droprate).to(torch.float32)
        # x = x * mask * (1/(1-droprate))    
        # #
        x = self.c_proj(x)
        x = self.dropout(x)
        return x
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.flash = hasattr(torch.nn.functional, "scaled_dot_product_attention") if 'flash' not in config.__dict__ else config.flash 
    ...
    def forward(self, x):
        ...
        if self.plot_probe:
            with torch.no_grad():
                self.att_for_plot += att.view(-1,self.n_head,T,T).mean(dim=0).cpu()
    ...
    def forward_dot_product_attn(self, k, q, v, B, T, C):
...
class BaseGPT(nn.Module):
    ...
    def generate(self, idx, max_new_tokens,storyid=None, temperature=1.0, top_k=None, stop_token=None, skip_future=True, future_topk=2):
        ...
        for _ in range(max_new_tokens):
            ...
            # print(idx_cond)
            logits, _ = self(idx_cond)
            # logits, _ = self.forward_with_attn_memory(idx_cond)

            # pluck the logits at the final step and scale by desired temperature
            logits = logits[:, -1, :] / temperature

            # print(logits[:,[3797, 6844,3290]])
            ...
            # except:
            #    print('warning network output is nan')
            #    break
...
class GPT(BaseGPT):
    ...
    def forward(self, idx, targets=None,storyid=None):
        device = idx.device
        b, t = idx.size()
        assert (
