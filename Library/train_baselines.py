...
$ python train_baselines.py --batch_size=32 

To run with DDP on 4 gpus on 1 node, example:
$ torchrun --standalone --nproc_per_node=4 train_baselines.py

To run with DDP on 4 gpus across 2 nodes, example:
- Run on the first (master) node with example IP xxx.xxx.xxx.xxx and example port xxx:
$ torchrun --nproc_per_node=8 --nnodes=2 --node_rank=0 --master_addr=xxx.xxx.xxx.xxx --master_port=xxx train_baselines.py
- Run on the worker node:
$ torchrun --nproc_per_node=8 --nnodes=2 --node_rank=1 --master_addr=xxx.xxx.xxx.xxx --master_port=xxx train_baselines.py
...
from baseline.baselines import GPT
